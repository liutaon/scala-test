 ...
 *
 * * How far - in terms of coordinate positions - the image should be inset from
 * the left hand border of a cell. * How far - in terms of coordinate positions
 * - the image should be inset from the from the top of the cell. * How far - in
 * terms of coordinate positions - the right hand edge of the image should
 * protrude into a cell (measured from the cells left hand edge to the images
 * right hand edge). * How far - in terms of coordinate positions - the bottom
 * edge of the image should protrude into a row (measured from the cells top
 * edge to the images bottom edge). * The index of the column that contains the
 * cell whose top left hand corner should be aligned with the top left hand
 ...
 *          the the SS usermodel classes. Ability to reuse the Drawing Patriarch
 *          so that multiple images can be inserted without unintentionally
 *          erasing earlier images. Check on image type added; i.e. jpg, jpeg or
 *          png. The String used to contain the files name is now converted into
 *          a URL. 2.10 17th May 2012 Corrected gross error that occurred when
 *          using the code with XSSF or SXSSF workbooks. In short, the code did
 *          not correctly calculate the size of the image(s) owing to the use of
 *          EMUs within the OOXML file format. That problem has largely been
 *          corrected although it should be mentioned that images are not sized
 *          with the same level of accuracy. Discrepancies of up to 2mm have
 *          been noted in testing. Further investigation will continue to
 *          rectify this issue.
 */
public class AddDimensionedImage {
	...
	public void addImageToSheet(String cellNumber, Sheet sheet, Drawing drawing, URL imageFile,
		...
		this.addImageToSheet(cellRef.getCol(), cellRef.getRow(), sheet, drawing, imageFile,
				reqImageWidthMM, reqImageHeightMM, resizeBehaviour);
	}
	...
	public void addImageToSheet(int colNumber, int rowNumber, Sheet sheet, Drawing drawing,
			URL imageFile, double reqImageWidthMM, double reqImageHeightMM, int resizeBehaviour)
			throws IOException, IllegalArgumentException {
		...
		colClientAnchorDetail = this.fitImageToColumns(sheet, colNumber, reqImageWidthMM,
				resizeBehaviour);
		rowClientAnchorDetail = this.fitImageToRows(sheet, rowNumber, reqImageHeightMM,
				resizeBehaviour);
		...
		int index = sheet.getWorkbook().addPicture(IOUtils.toByteArray(imageFile.openStream()),
				imageType);
	...
	private ClientAnchorDetail fitImageToColumns(Sheet sheet, int colNumber,
		...
		if (colWidthMM < reqImageWidthMM) {
			...
			if ((resizeBehaviour == AddDimensionedImage.EXPAND_COLUMN)
				...
				colClientAnchorDetail = new ClientAnchorDetail(colNumber, colNumber,
						pictureWidthCoordinates);
			}
			...
			// the image out across one or more columns.
					...
					|| (resizeBehaviour == AddDimensionedImage.EXPAND_ROW)) {
				colClientAnchorDetail = this.calculateColumnLocation(sheet, colNumber,
						reqImageWidthMM);
			}
		}
	...
	private ClientAnchorDetail fitImageToRows(Sheet sheet, int rowNumber, double reqImageHeightMM,
		...
		if (rowHeightMM < reqImageHeightMM) {
			if ((resizeBehaviour == AddDimensionedImage.EXPAND_ROW)
				...
				rowClientAnchorDetail = new ClientAnchorDetail(rowNumber, rowNumber,
						pictureHeightCoordinates);
			}
			// If the user has chosen to overlay both rows and columns or just
			...
			// the image out ver one or more rows.
				...
				rowClientAnchorDetail = this.calculateRowLocation(sheet, rowNumber,
						reqImageHeightMM);
			}
		}
	 ...
	 *         number of coordinate positions.
	 */
	private ClientAnchorDetail calculateColumnLocation(Sheet sheet, int startingColumn,
	 ...
	 */
	private ClientAnchorDetail calculateRowLocation(Sheet sheet, int startingRow,
